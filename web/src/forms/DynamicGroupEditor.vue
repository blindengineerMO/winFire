...
const props=defineProps({modelValue:{type:Object,default:()=>({enabled:false,match:'all',rules:[]})},disabled:{type:Boolean,default:false}})
...
<template>
  <section class="dynamic-group-editor">
    <div class="dynamic-group-heading"><div><span class="eyebrow">DYNAMIC MEMBERSHIP</span><h3>Update group from node attributes</h3></div><label class="training-check"><input v-model="draft.enabled" type="checkbox" :disabled="disabled" @change="update"> Enable rules</label></div>
...
<style scoped>
.dynamic-group-editor{display:grid;gap:.75rem;padding:1rem;border:1px solid var(--border);border-radius:9px;background:color-mix(in srgb,var(--panel) 70%,transparent)}.dynamic-group-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:1rem}.dynamic-group-heading h3{margin:.2rem 0 0}.training-check{display:flex;align-items:center;gap:.5rem;white-space:nowrap}.training-check input{width:17px;height:17px}.dynamic-rule-toolbar{display:grid;grid-template-columns:1fr 1.2fr 1.1fr 2fr auto;gap:.6rem;align-items:end}.dynamic-rule-toolbar label{display:grid;gap:.3rem;font-size:.72rem;font-weight:700}.dynamic-rule-toolbar input,.dynamic-rule-toolbar select{min-width:0}.dynamic-rule-toolbar.disabled{opacity:.6}.dynamic-rule-list{display:grid;gap:.4rem}.dynamic-rule{display:flex;align-items:center;gap:.55rem;flex-wrap:wrap;padding:.5rem .65rem;border:1px solid var(--border);border-radius:7px}.dynamic-rule code{overflow-wrap:anywhere;color:var(--field-value)}.dynamic-rule .icon-button{margin-left:auto}@media(max-width:800px){.dynamic-rule-toolbar{grid-template-columns:repeat(2,minmax(0,1fr))}.dynamic-rule-toolbar .dynamic-value{grid-column:span 2}.dynamic-rule-toolbar button{grid-column:span 2}}
</style>
