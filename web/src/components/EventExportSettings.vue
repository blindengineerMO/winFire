...
const destinations=ref([]),error=ref(''),message=ref(''),busy=ref(false),removeTarget=ref(null),removeConfirmed=ref(false)
...
function askRemove(destination){removeTarget.value=destination;removeConfirmed.value=false}
async function remove(){if(!removeTarget.value||!removeConfirmed.value)return;busy.value=true;error.value='';try{await api(`/event-export/destinations/${removeTarget.value.id}`,{method:'DELETE'});message.value='Destination deleted';if(form.value.id===removeTarget.value.id)reset();removeTarget.value=null;await load()}catch(cause){error.value=cause.message}finally{busy.value=false}}
onMounted(load)
</script>
...
<template>
  <section class="export-settings">
    ...
    <GlassWindow v-if="removeTarget" :model-value="true" title="Delete export destination" width="480px" @update:model-value="value=>{if(!value)removeTarget=null}"><div class="confirm-dialog"><p>Delete <strong>{{removeTarget.name}}</strong> as an event export destination?</p><label class="check-label"><input v-model="removeConfirmed" type="checkbox"> I understand this destination will be deleted.</label><div class="export-actions"><button class="button small secondary" @click="removeTarget=null;removeConfirmed=false">Cancel</button><button class="button small danger" :disabled="busy||!removeConfirmed" @click="remove">{{busy?'Deleting…':'Delete destination'}}</button></div></div></GlassWindow>
  </section>
</template>

<style scoped>
.export-settings{border-top:1px solid var(--border);padding:1.2rem 0;display:grid;gap:.65rem}.export-settings h3,.export-form h4{margin:0}.export-settings p{margin:.2rem 0}.export-destination{display:flex;align-items:center;gap:.5rem;padding:.6rem;border:1px solid var(--border);border-radius:7px}.export-destination div{display:grid;gap:.2rem;min-width:0;flex:1}.export-destination small{overflow-wrap:anywhere;color:var(--muted)}.export-form{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:.7rem;padding:.9rem;border:1px solid var(--border);border-radius:7px}.export-form h4,.export-actions{grid-column:1/-1}.export-form label{display:grid;gap:.3rem;font-size:.75rem}.export-form small{color:var(--muted)}.export-actions{display:flex;gap:.5rem;flex-wrap:wrap}.confirm-dialog{display:grid;gap:1rem}
</style>
