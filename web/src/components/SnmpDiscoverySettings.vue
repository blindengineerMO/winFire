...
function requestRemove(target){removeTarget.value=target;confirmOpen.value=true}
async function remove(){const target=removeTarget.value;confirmOpen.value=false;if(!target)return;busy.value=true;try{await api(`/discovery/snmp-targets/${target.id}`,{method:'DELETE'});await load()}catch(e){error.value=e.message}finally{busy.value=false;removeTarget.value=null}}
...
<template>
  ...
  <section v-else class="snmp-settings">
    ...
    <form class="form-grid" @submit.prevent="save">
      ...
      <label class="switch-field"><span>Enable scheduled polling</span><input v-model="form.enabled" type="checkbox" role="switch"><span class="switch-control" aria-hidden="true"></span></label>
    ...
    <div class="table-wrap"><table><thead><tr><th>TARGET</th><th>HOST</th><th>SCOPE</th><th>CREDENTIAL</th><th>LAST POLL</th><th>RESULT</th><th>ACTIONS</th></tr></thead><tbody>
      <tr v-for="target in targets" :key="target.id"><td><strong>{{target.name}}</strong><small>{{target.enabled?'Scheduled':'Paused'}} · every {{target.pollIntervalMinutes}}m</small></td><td class="mono">{{target.host}}</td><td class="mono">{{target.cidr||'All ARP entries'}}</td><td>{{target.credentialName}}</td><td>{{target.lastPollAt?new Date(target.lastPollAt).toLocaleString():'Never'}}</td><td><span class="status" :class="target.lastStatus==='complete'?'reachable':target.lastStatus==='failed'?'unreachable':'unknown'">{{target.lastStatus||'Pending'}}</span><small v-if="target.lastStatus==='complete'">{{target.arpCount}} ARP · {{target.macPortCount}} MAC ports · {{target.registeredCount}} registered</small><small v-if="target.lastError" class="danger-text">{{target.lastError}}</small></td><td><div class="inline-actions"><button class="button small secondary" :disabled="busy" @click="poll(target)">Poll now</button><button class="button small danger" :disabled="busy" @click="requestRemove(target)">Remove</button></div></td></tr>
      <tr v-if="!targets.length"><td colspan="7" class="empty-table">No SNMP discovery targets configured.</td></tr>
    </tbody></table></div>
  </section>
  <ConfirmDialog v-model="confirmOpen" title="Remove SNMP target" :message="`Remove ${removeTarget?.name||'this SNMP target'}? Poll history will be retained.`" confirm-label="Remove target" @confirm="remove" />
</template>
<style scoped>.snmp-settings{display:grid;gap:1rem}.snmp-settings .muted,.snmp-settings .hint{padding:0 1rem}.snmp-settings .form-grid{padding:0 1rem}.snmp-settings td small{display:block;color:var(--muted);font-size:.7rem;margin-top:.25rem}.snmp-settings .danger-text{color:var(--danger)}.snmp-oid-reference{margin:0 1rem;padding:.75rem 1rem;border:1px solid var(--border);border-radius:7px;color:var(--muted);font-size:.7rem}.snmp-oid-reference summary{cursor:pointer;color:var(--field-value);font-weight:700}.snmp-oid-reference p{line-height:1.5;margin:.55rem 0 0}.snmp-oid-reference code{font-family:var(--mono,'JetBrains Mono',monospace);color:var(--green)}</style>
