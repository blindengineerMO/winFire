...
async function save(){
  ...
  try{settings.value=await api('/settings/wef',{method:'PATCH',body:{enabled:settings.value.enabled}});message.value=settings.value.enabled?'WEF push receiver enabled':'WEF push receiver disabled'}catch(cause){error.value=cause.message}finally{busy.value=false}
}
...
<template>
  <section class="wef-settings">
    ...
    <div class="wef-status"><span class="status" :class="settings.enabled&&settings.secretConfigured?'reachable':'unknown'">{{settings.enabled?'Enabled':'Disabled'}}</span><span>{{settings.secretConfigured?'Shared secret configured':'Set WEF_SHARED_SECRET on the server'}}</span><code>{{settings.baseUrl||'PUBLIC_BASE_URL'}}{{settings.path}}</code></div>
    <div class="inline-actions"><label class="check-label"><input v-model="settings.enabled" type="checkbox"> Enable push receiver</label><button class="button small primary" :disabled="busy||!settings.secretConfigured" @click="save">Save receiver setting</button></div>
