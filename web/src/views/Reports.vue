...
import CapabilityReport from '../components/CapabilityReport.vue'
...
async function load(){if(tab.value==='capabilities')return;busy.value=true;try{rows.value=await api(`/reports/${tab.value}`);error.value=''}catch(e){error.value=e.message}finally{busy.value=false}}
...
<template><div class="view"><PageHeader eyebrow="ANALYTICS / COMPLIANCE" title="Reports" description="Coverage, inventory and verification evidence"><button v-if="canEdit&&tab==='coverage'" class="button secondary" :disabled="busy" @click="checkDrift"><i class="mdi mdi-radar"></i> Check drift</button><button v-if="tab!=='capabilities'" class="button secondary" @click="exportFile('csv')"><i class="mdi mdi-file-delimited-outline"></i> CSV</button><button v-if="tab!=='capabilities'" class="button primary" @click="exportFile('pdf')"><i class="mdi mdi-file-pdf-box"></i> PDF</button></PageHeader><div v-if="error" class="error-msg">{{error}}</div><div v-if="message" class="success-msg">{{message}}</div><section class="panel glass"><div class="tab-bar"><button v-for="name in ['coverage','capabilities','inventory','dns','compliance','verification']" :key="name" :class="{active:tab===name}" @click="tab=name;load()">{{name}}</button></div><CapabilityReport v-if="tab==='capabilities'"/><template v-else><div class="panel-title"><div><span class="eyebrow">{{tab.toUpperCase()}} REPORT</span><h2>{{rows.length}} records</h2></div><input v-model="search" class="search" placeholder="Filter report" aria-label="Filter report"></div><div class="table-wrap"><table><thead><tr><th v-for="column in columns" :key="column">{{column.replaceAll('_',' ').toUpperCase()}}</th></tr></thead><tbody><tr v-for="(row,index) in filtered" :key="index"><td v-for="column in columns" :key="column">{{row[column]??'—'}}</td></tr><tr v-if="!filtered.length"><td :colspan="columns.length||1" class="empty-table">{{busy?'Loading report…':'No matching records.'}}</td></tr></tbody></table></div></template></section></div></template>
