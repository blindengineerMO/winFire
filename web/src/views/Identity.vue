...
const form=ref({name:'',nodeId:'',nodeGroupId:'',policyId:null,port:3389,accountSid:'',sourceIp:'',extraPorts:[],sourceProcess:'',fallbackToLoggedOnUser:false,ttlMinutes:240,failOpen:false,mode:'agentless',mfaProvider:'totp',portalEnabled:true,autoPromptEnabled:false})
const extraPortsText=ref(''),allowedEmails=ref(''),manageSegment=ref(null),manageEmails=ref(''),manageProvider=ref('totp'),manageEnabled=ref(true),manageTtl=ref(240),manageSource=ref(''),managePolicyId=ref(null),manageAutoPrompt=ref(false),manageAccountSid=ref(''),manageExtraPorts=ref(''),manageSourceProcess=ref(''),manageFallback=ref(false),manageFailOpen=ref(false)
const lsaReason=ref(''),lsaConfirmation=ref(''),lsaBusy=ref(false)
...
async function setLsaBaseline(enabled){
  ...
  try{
    await api(`/segments/${manageSegment.value.id}/lsa-baselines`,{method:'POST',body:{enabled,reason:lsaReason.value,confirmation:lsaConfirmation.value}})
    await load();manageSegment.value=segments.value.find(item=>item.id===manageSegment.value.id)||manageSegment.value
    lsaReason.value='';lsaConfirmation.value='';message.value=enabled?'LSA deny baseline enforced and verified on the segment targets.':'Original LSA assignments restored and verified.'
  }catch(cause){error.value=cause.message}
...
async function changeLogonRight(){
  ...
  try{
    ...
    rightsOpen.value=false;rightsForm.value={nodeId:'',accountSid:'',right:'SeRemoteInteractiveLogonRight',present:true,reason:'',confirmation:''}
...
<template>
  <div class="view">
    ...
    <section class="panel glass"><div class="panel-title"><div><span class="eyebrow">WINDOWS LSA</span><h2>Logon rights baseline</h2></div><div v-if="canManage" class="rights-actions"><select @change="baseline($event.target.value);$event.target.value=''" aria-label="Collect node baseline"><option value="">Collect from node…</option><option v-for="node in nodes.filter(item=>['winrm','winrms'].includes(item.transport))" :key="node.id" :value="node.id">{{node.hostname}}</option></select><button class="button secondary" @click="rightsOpen=true">Change logon right</button></div></div><div class="table-wrap"><table><thead><tr><th>NODE</th><th>ACCOUNT</th><th>SOURCE</th><th>ACCOUNT SID</th><th>LOGON TYPE</th><th>ASSIGNMENT</th><th>RECORDED</th></tr></thead><tbody><tr v-for="right in rights.slice(0,100)" :key="right.id"><td>{{nodes.find(node=>node.id===right.node_id)?.hostname||right.node_id}}</td><td>{{right.accountName||'Unresolved'}}</td><td>{{right.accountSource||'—'}}</td><td class="mono">{{right.account_sid}}</td><td class="mono">{{right.logon_type}}</td><td>{{right.right_assignment}}</td><td>{{new Date(right.at).toLocaleString()}}</td></tr><tr v-if="!rights.length"><td colspan="7" class="empty-table">No baselines collected yet.</td></tr></tbody></table></div></section>
