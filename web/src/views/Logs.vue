...
function canIgnoreTraffic(event){return [5150,5151,5156,5157].includes(Number(event.event_id))||event.event_type==='firewall'}
...
<template>
  <div class="view">
    ...
    <nav class="view-tabs" aria-label="Firewall events sections"><button type="button" :class="{active:activeTab==='events'}" @click="activeTab='events'">Firewall events <span>{{total}}</span></button><button type="button" :class="{active:activeTab==='learning'}" @click="activeTab='learning'">Learning sessions <span>{{sessions.length}}</span></button></nav>
    <section v-if="activeTab==='events'" class="panel glass">
...
.service-cell{max-width:220px;white-space:normal}.service-cell strong{display:block;font-size:.72rem}.service-cell small{display:block;color:var(--muted);font-size:.64rem;line-height:1.25;margin-top:.2rem;max-width:220px}
