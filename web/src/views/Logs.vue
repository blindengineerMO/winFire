  ...
  <div class="view">
    ...
    <section class="panel glass">
      ...
      <div class="table-wrap events-table-wrap"><table class="events-table"><thead>
        <tr><th v-if="canManageRules" aria-label="Select events"></th><th v-for="column in columns" :key="column.key" :aria-sort="sortBy===column.key?(sortDir==='asc'?'ascending':'descending'):undefined"><button type="button" class="sort-heading" :aria-label="`Sort by ${column.label}`" @click="sort(column.key)">{{column.label}} <i :class="sortBy===column.key?(sortDir==='asc'?'mdi mdi-arrow-up':'mdi mdi-arrow-down'):'mdi mdi-swap-vertical'"></i></button></th><th v-if="canManageRules">Actions</th></tr>
        <tr class="column-filters">
          <th v-if="canManageRules"></th>
