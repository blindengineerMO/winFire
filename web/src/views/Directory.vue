...
const accountForm=ref({reason:'',confirmation:'',durationMinutes:60,temporary:false})
...
async function openDetail(user){try{detail.value=await api(`/directory/users/${user.id}`);role.value='auditor';accountForm.value={reason:'',confirmation:'',durationMinutes:60,temporary:false};detailOpen.value=true}catch(cause){error.value=cause.message}}
...
async function setAccountStatus(){if(!detail.value)return;busy.value=true;error.value='';const enabled=!detail.value.enabled;try{const result=await api(`/directory/users/${detail.value.id}/status`,{method:'POST',body:{enabled,reason:accountForm.value.reason,confirmation:accountForm.value.confirmation,...(!enabled&&accountForm.value.temporary?{durationMinutes:Number(accountForm.value.durationMinutes)}:{})}});message.value=`${detail.value.display_name} ${enabled?'enabled':'disabled'} in AD${result.expiresAt?` until ${new Date(result.expiresAt).toLocaleString()}`:''}`;detail.value=await api(`/directory/users/${detail.value.id}`);accountForm.value={reason:'',confirmation:'',durationMinutes:60,temporary:false};await load()}catch(cause){error.value=cause.message}finally{busy.value=false}}
onMounted(load)
</script>
...
<template>
  <div class="view directory-view">
    ...
    <GlassWindow v-model="detailOpen" :title="detail?.display_name||'Directory user'" width="760px"><div v-if="detail" class="directory-detail">
      ...
      <form v-if="isAdmin&&!detail.missing" class="account-control" @submit.prevent="setAccountStatus"><h3>AD account control</h3><label>Reason<textarea v-model.trim="accountForm.reason" minlength="10" maxlength="500" rows="2" required placeholder="Reason for changing this account"></textarea></label><label v-if="detail.enabled" class="account-checkbox"><input v-model="accountForm.temporary" type="checkbox"> Re-enable automatically after a set time</label><label v-if="detail.enabled&&accountForm.temporary">Disable for (minutes)<input v-model.number="accountForm.durationMinutes" type="number" min="5" max="10080" required></label><label>Type {{detail.enabled?'DISABLE AD USER':'ENABLE AD USER'}} to confirm<input v-model.trim="accountForm.confirmation" autocomplete="off" required></label><button class="button small danger" :disabled="busy||accountForm.confirmation!==(detail.enabled?'DISABLE AD USER':'ENABLE AD USER')">{{detail.enabled?'Disable in AD':'Enable in AD'}}</button></form>
