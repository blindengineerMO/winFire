...
const links=[['/','view-dashboard-outline','Overview'],['/inventory','server-network','Inventory'],['/directory','account-group-outline','Directory'],['/policies','source-branch','Policy Studio'],['/reports','chart-box-outline','Reports'],['/logs','text-box-search-outline','Event logs'],['/internet','web','Internet'],['/identity','shield-account-outline','Identity'],['/admin','cog-outline','Administration']]
...
const enterpriseTitle=computed(()=>({'/':'Dashboard','/inventory':'Network / Assets','/directory':'Entities / Directory','/policies':'Network / Policies','/reports':'Reporting','/logs':'Network / Visibility','/internet':'Network / Internet','/identity':'Identity','/admin':'Administration'})[route.path]||title.value)
...
<template>
  ...
  <div v-else class="shell">
    ...
    <aside class="enterprise-side" aria-label="Enterprise navigation" :class="{'side-open':menuOpen}">
      ...
      <nav class="enterprise-nav" aria-label="Enterprise navigation">
        ...
        <div class="enterprise-nav-group"><span class="enterprise-nav-label">VISIBILITY</span><router-link to="/logs" @click="menuOpen=false"><i class="mdi mdi-eye-outline"></i> Activities</router-link><router-link to="/internet" @click="menuOpen=false"><i class="mdi mdi-web"></i> Internet</router-link></div>
      </nav>
