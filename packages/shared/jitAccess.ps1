...
function Test-WinFireJitGate($data) {
  ...
  $overlaps=@(Get-NetFirewallRule -PolicyStore ActiveStore -Enabled True -Direction Inbound -ErrorAction Stop | Where-Object { $_.Group -notlike 'WinFireSecure:JIT:*' -and $_.Group -ne 'WinFireSecure:system-loopback' } | ForEach-Object {
    ...
    $matched=$false
    foreach($part in (([string]$filter.LocalPort) -split ',')) {
      $token=$part.Trim()
  ...
  [pscustomobject]@{safe=($closed -and $overlaps.Count -eq 0);profileReady=$closed;profiles=$profiles;conflictingAllows=$allows;conflictingBlocks=$blocks}
}
function Start-WinFireJitAccess($data) {
  ...
  try {
    ...
    $registered=$true
    New-NetFirewallRule -DisplayName ('WinFire MFA '+$identity.group) -Group $identity.group -Direction Inbound -Action Allow -Protocol TCP -LocalPort ([int]$data.port) -RemoteAddress ([string]$source) -Profile Any -ErrorAction Stop | Out-Null
    ...
    [pscustomobject]@{active=$true;group=$identity.group;task=$identity.task;expiresAt=$expiry.UtcDateTime.ToString('o')}
  } catch {
    $cause=$_
