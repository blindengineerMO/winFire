...
const names=['discovery','authentication','facts','firewallRead','firewallWrite','policyReadback','events','verification']
const defaults={discovery:168,authentication:24,facts:24,firewallRead:24,firewallWrite:168,policyReadback:24,events:2,verification:24}
...
export function nodeCoverage(node,{at=Date.now(),settings=coverageSettings()}={}){
  ...
  const raw={discovery:latest(node.first_discovered_at,node.last_seen_at,one('SELECT MAX(fetched_at) at FROM asset_sources WHERE node_id=?',node.id)?.at),authentication:latest(auth?.success,agent&&!agent.revoked_at?agent.last_checkin_at:null),facts:facts?.collected_at,events:cursor?.updated_at,verification:one('SELECT MAX(run_at) at FROM verifier_results WHERE node_id=? AND passed=1',node.id)?.at}
  const supported={discovery:true,authentication:host||snmp||api,facts:host||snmp||api,firewallRead:host,firewallWrite:host,policyReadback:host,events:host&&node.transport!=='ssh',verification:host}
  const capabilities={}
  for(const key of names){
    const entries=evidence.filter(e=>e.capability===key),success=latest(raw[key],...entries.map(e=>e.last_success_at)),failure=latest(key==='authentication'?auth?.failure:null,...entries.map(e=>e.last_failure_at)),fresh=success&&Number.isFinite(Date.parse(success))&&at-Date.parse(success)<=settings[key]*3600000
    const state=!supported[key]?'unsupported':failure&&(!success||Date.parse(failure)>Date.parse(success))?'failed':!success?'missing':fresh?'fresh':'stale'
    capabilities[key]={state,lastSuccessAt:success,lastFailureAt:failure,thresholdHours:settings[key],sources:entries.map(e=>e.source),remediation:state==='fresh'?null:key==='authentication'?'/admin':'/inventory?node='+node.id}
  }
  const gaps=names.filter(k=>supported[k]&&capabilities[k].state!=='fresh')
  return {nodeId:node.id,hostname:node.hostname,address:node.ip,scopeId:node.scope_id||'default',transport:node.transport||node.connection_mode||'unknown',source:node.inventory_source,eligible:isLocalAssetNode(node),cloudObservationOnly:cloud&&!host&&!snmp&&!api,capabilities,gaps,learningReady:host&&['authentication','events'].every(k=>capabilities[k].state==='fresh')}
}
export function fleetCoverage({q='',scopeId,transport,source,gap,page=1,pageSize=25}={}){
  ...
  const rows=eligible.filter(n=>(!scopeId||n.scope_id===scopeId)&&(!transport||n.transport===transport)&&(!source||n.inventory_source===source)&&(!term||[n.hostname,n.ip,n.os_name].join(' ').toLowerCase().includes(term))).map(n=>nodeCoverage(n,{settings})).filter(n=>!gap||n.gaps.includes(gap)).sort((a,b)=>b.gaps.length-a.gaps.length||a.hostname.localeCompare(b.hostname)||a.nodeId.localeCompare(b.nodeId))
  return {items:rows.slice((page-1)*pageSize,page*pageSize),total:rows.length,page,pageSize,eligibleDenominator:eligible.length,excluded:nodes.length-eligible.length,summary:Object.fromEntries(names.map(k=>[k,{supported:rows.filter(n=>n.capabilities[k].state!=='unsupported').length,fresh:rows.filter(n=>n.capabilities[k].state==='fresh').length}])),thresholdHours:settings}
}
