...
export function previewSecurityAutomation(policy,{limit=100,offset=0,upperBound=now(),ascending=false}={}){
  ...
  const accountAction=policy.action_type==='disable_ad'
  const items=all(`SELECT ${accountAction?'verified_account_sid subject_sid,MIN(lower(user_upn)) user_upn':'lower(user_upn) user_upn'},COUNT(*) failures,MAX(resolved_at) latest
    FROM mfa_challenges WHERE status='denied' AND resolved_at>=? AND resolved_at>=? AND resolved_at<=?
    AND user_upn IS NOT NULL ${accountAction?"AND failure_kind='totp' AND verified_account_sid IS NOT NULL":''}
    GROUP BY ${accountAction?'verified_account_sid':'lower(user_upn)'} HAVING COUNT(*)>=?
    ORDER BY ${accountAction?'subject_sid':'user_upn'} ${ascending?'ASC':'DESC'} LIMIT ? OFFSET ?`,cutoff(policy.window_minutes),policy.created_at||'1970-01-01',upperBound,policy.failure_count,limit,offset)
  return {count:items.length,items}
}
...
async function disableVerifiedAccount(policy,subject,eventRef,detail,node,event,{logoff=async(target,args)=>remote(target,'security_session_logoff',args),writeStatus=writeDirectoryUserStatus}={}){
  ...
  try{
    const result=await writeStatus(settings,actionCredential,user,false)
    ...
    try{
      ...
      try{await writeStatus(settings,actionCredential,user,true,{expectedUac:result.afterUac})}
    ...
    if(policy.action_type==='disable_ad_logoff'){
      ...
      audit(null,'security.automation.client-logoff.start','node',node.id,null,{policyId:policy.id,sessionId:detail.sessionId,runId:applyRunId})
      try{
        ...
        run('UPDATE policy_apply_runs SET status=?,diff_json=?,finished_at=? WHERE id=?','success',json({operation:'security_session_logoff',policyId:policy.id,sessionId:detail.sessionId,accountSid:detail.accountSid}),now(),applyRunId)
        audit(null,'security.automation.client-logoff.success','node',node.id,null,{policyId:policy.id,sessionId:detail.sessionId,accountSid:detail.accountSid,runId:applyRunId})
      }catch(error){
        ...
        audit(null,'security.automation.client-logoff.failed','node',node.id,null,{policyId:policy.id,sessionId:detail.sessionId,runId:applyRunId,error:error.message})
      }
    }
...
export async function processSecurityAutomations({verifyOwner=async(node,event)=>remote(node,'security_process_owner',{processId:event.process_id,eventTime:event.event_time}),logoff=async(node,args)=>remote(node,'security_session_logoff',args),writeStatus=writeDirectoryUserStatus}={}){
  let triggered=0
  ...
  for(const policy of all('SELECT * FROM security_automations WHERE enabled=1 ORDER BY created_at')){
    if(policy.trigger_type==='mfa_failures'){
      ...
      for(const row of rows){
        const latest=policy.action_type==='disable_ad'
          ?one("SELECT id FROM mfa_challenges WHERE status='denied' AND failure_kind='totp' AND verified_account_sid=? AND resolved_at=? ORDER BY rowid DESC LIMIT 1",row.subject_sid,row.latest)
          :one("SELECT id FROM mfa_challenges WHERE status='denied' AND lower(user_upn)=? AND resolved_at=? ORDER BY rowid DESC LIMIT 1",row.user_upn,row.latest)
        if(!latest)continue
        const detail={summary:`${row.failures} failed MFA challenges for ${row.user_upn} in ${policy.window_minutes} minutes`,failures:row.failures,accountSid:row.subject_sid||null}
        const fired=policy.action_type==='disable_ad'
          ?await disableVerifiedAccount(policy,row.subject_sid,latest.id,detail,null,null,{writeStatus})
          :emitIncident(policy,row.user_upn,latest.id,detail,'alert')
        if(fired)triggered++
      }
    ...
    for(const event of events){
      ...
      if(policy.action_type==='disable_ad'||policy.action_type==='disable_ad_logoff'){
      ...
      const fired=policy.action_type!=='alert'&&owner?.sid
        ?await disableVerifiedAccount(policy,subject,event.id,detail,one('SELECT * FROM nodes WHERE id=?',event.node_id),event,{logoff,writeStatus})
