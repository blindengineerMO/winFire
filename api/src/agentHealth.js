...
export function sweepAgentHealth(maxSilenceMs=120_000) {
  ...
  const offline=all(`SELECT n.id AS node_id,n.agent_id FROM nodes n LEFT JOIN agents a ON a.id=n.agent_id WHERE n.connection_mode='agent' AND n.status!='unreachable' AND (a.id IS NULL OR a.revoked_at IS NOT NULL OR a.last_checkin_at IS NULL OR a.last_checkin_at<?)`,cutoff)
