...
export function persistHypervisor(nodeId,hypervisor){
  ...
  const classificationEvidence=hypervisor.classificationEvidence||{source:'hypervisor-discovery',method:esxi?'vSphere SOAP':'hypervisor fingerprint',matched:name,signal:hypervisor.evidence||null,confidence:hypervisor.authenticated?'high':'medium'}
  run("UPDATE nodes SET os_name=?,os_version=COALESCE(?,os_version),platform=?,vendor=COALESCE(?,vendor),classification_evidence_json=?,hypervisor=?,device_type=?,management_type=?,manageability='unmanaged',transport=?,connection_mode='agentless',agent_required=0,status='reachable',firewall_state='unmanaged',snmp_capable=1,probe_status=?,last_probe_at=? WHERE id=?",name,version,name,hypervisor.vendor||name,JSON.stringify(classificationEvidence),hypervisor.hypervisor||name,esxi?'esxi':'hypervisor',esxi?'api':'manual',transport,hypervisor.authenticated?'hypervisor-authenticated':'hypervisor-detected',now(),nodeId)
  run("INSERT INTO node_facts(node_id,snapshot_json,collected_at) VALUES(?,?,?) ON CONFLICT(node_id) DO UPDATE SET snapshot_json=excluded.snapshot_json,collected_at=excluded.collected_at",nodeId,json({source:transport,hypervisor:{...hypervisor,classificationEvidence},virtualMachines:Array.isArray(hypervisor.virtualMachines)?hypervisor.virtualMachines:[]}),now())
