...
function ensureSnmpNode(target,device,stamp){
  ...
  const firewallState=deviceType==='hypervisor'?'unmanaged':['firewall','switch','router'].includes(deviceType)?'learning':'unmanaged'
  const osName=classification.vendor||null,osVersion=identity.sysDescr||null
  let nodeId=existing?.id
  if(existing){
    run("UPDATE nodes SET hostname=COALESCE(NULLIF(?,''),hostname),ip=COALESCE(ip,?),connection_mode='snmp',transport='snmp',status='reachable',agent_required=0,snmp_capable=1,device_type=?,management_type='snmp',manageability=?,firewall_state=?,hypervisor=CASE WHEN ?='hypervisor' THEN COALESCE(?,hypervisor) ELSE hypervisor END,os_name=COALESCE(NULLIF(?,''),os_name),os_version=COALESCE(NULLIF(?,''),os_version),last_seen_at=?,last_discovered_at=?,last_probe_at=?,probe_status='snmp-authenticated',discovery_source=? WHERE id=?",hostname,target.host,deviceType,manageability,firewallState,deviceType,classification.hypervisor||classification.vendor||null,osName,osVersion,stamp,stamp,stamp,`snmp:${target.id}`,existing.id)
  }else{
    nodeId=id()
    run("INSERT INTO nodes(id,hostname,fqdn,ip,connection_mode,transport,status,inventory_source,discovery_source,last_seen_at,last_discovered_at,last_probe_at,probe_status,agent_required,firewall_state,snmp_capable,device_type,management_type,manageability,hypervisor,os_name,os_version) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,0,?,1,?,?,?,?,?,?)",nodeId,hostname,null,target.host,'snmp','snmp','reachable','snmp',`snmp:${target.id}`,stamp,stamp,stamp,'snmp-authenticated',firewallState,deviceType,'snmp',manageability,classification.deviceType==='hypervisor'?(classification.hypervisor||classification.vendor):null,osName,osVersion)
  }
