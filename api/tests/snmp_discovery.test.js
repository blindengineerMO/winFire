...
test('SNMP credentials validate v2c and v3 secrets without exposing them',async()=>{
  ...
  const edited=await auth(request.patch(`/api/v1/nodes/${node.body.id}`)).send({deviceType:'esxi',managementType:'manual',credentialIds:[esxiCredential.body.id]}).expect(200)
  assert.equal(edited.body.device_type,'esxi')
  assert.equal(edited.body.hypervisor,'VMware ESXi')
  assert.equal(edited.body.management_type,'manual')
