...
test('learning proposals preserve traffic direction and require approval before application',async()=>{
  ...
  const {capabilityEvidence:recordCollectorHealth}=await import('../src/services/capabilities.js')
  recordCollectorHealth(node.body.id,'events','agent-collector',{detail:{caughtUp:true,successAuditEnabled:true}})
...
test('automatic training remains pending without a working log transport and ends cleanly with no traffic',async()=>{
  ...
  const noCollector=await processDueTraining()
  assert.ok(noCollector.some(result=>result.sessionId===agentNode.body.training.id&&result.status==='failed'))
  assert.match(db.prepare('SELECT last_error FROM learning_sessions WHERE id=?').get(agentNode.body.training.id).last_error,/collection health/)
  const {capabilityEvidence:recordCollectorHealth}=await import('../src/services/capabilities.js')
  recordCollectorHealth(agentNode.body.id,'events','agent-collector',{detail:{caughtUp:true,successAuditEnabled:true}})
  db.prepare('UPDATE learning_sessions SET last_attempt_at=NULL WHERE id=?').run(agentNode.body.training.id)
