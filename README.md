...
- New nodes start automatic training for 30 days by default. Administration can set 1–365 days for future nodes. At the deadline, the server collects the last WinRM event page or checks that the enrolled agent is online, compiles new observed allow flows, assigns a learned policy, and applies it. Agent nodes become enforced after the agent acknowledges the apply. Inventory can start a timed relearning window; previously assigned rules remain active, and duplicate observed flows are skipped. Manual review sessions remain available in Event logs.
