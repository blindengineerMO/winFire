...
- Admins can right-click an AD-discovered, unenrolled inventory row (or use its actions button) to open agent enrollment. The dialog generates a 15-minute token and gives installation steps. Token generation requires HTTPS and configured agent PKI; the signed agent must then be installed on the Windows host.
- Admins can right-click a WinRM or enrolled-agent node and choose **Break glass**. A reason, 5–240 minute duration, and typed confirmation are required. The host snapshots Domain, Private, and Public firewall profile states, schedules a SYSTEM rollback task locally, then temporarily disables the profiles; explicit block rules would otherwise override allow-all rules. The control plane can restore early and sweeps expired sessions. Activation and restoration are audited. A reachable Windows host with permission to manage its firewall and Scheduled Tasks is required. The script has automated mock PowerShell coverage; host execution should be validated in a safe Windows test environment before operational use.
...
- Administration → Directory configures an LDAPS domain controller, search base, vault bind credential, optional WinRM credential for discovered computers, and sync interval. An admin can test the bind and run a sync immediately. AD computer GUIDs anchor inventory across renames; missing computers are marked, and existing nodes and policies are retained. Enabled discovered computers start new-host training. The bind uses certificate validation and does not store a directory password in settings. Account control actions are not implemented.
...
- The agentless **WinFire access portal** takes a fresh authenticator code or a fresh Entra sign-in, checks that the target's effective firewall profiles are enabled with inbound default Block and no other enabled allow rule for the requested TCP port, then adds one source-IP-scoped temporary allow rule. A SYSTEM scheduled task on the Windows host removes it at expiry even if the control plane is offline; the control plane also retries expiry cleanup. Users can close their grant early. TOTP codes are single-use for portal grants. Entra requires a tenant app configured to emit the `amr` ID-token claim and an MFA Conditional Access policy; it has not been validated against a live tenant.
- Administration → Branding sets the company name and uploads a validated PNG, JPEG, or WebP image for the MFA page and its prompted login. An administrator can opt a segment into automatic browser prompts. A short interval collector matches inbound blocked 5157 traffic to a managed or AD discovered workstation, then matches a recent outbound 5156 TCP event, source port, process owner, and active session on that workstation. A one-time interactive Scheduled Task opens the fixed HTTPS portal URL for that user. The portal displays the matched workstation, user, and session from the server-side prompt record. AD discovered clients use the Directory WinRM credential when no node credential is assigned. Disconnected sessions and ambiguous process ownership remain blocked. Administration → Branding also controls an explicit, default-off fail-open fallback for unknown or WinRM-unreachable clients. Enabling it requires typed approval and creates a 2–15 minute audited source-IP grant without MFA; the target firewall preflight must still pass. See [the Microsoft-based design notes](RESEARCH_AGENTLESS_MFA.md).
...
- The portal opens network access before the user retries RDP or SSH. The optional browser prompt reacts to a blocked event after polling; it cannot intercept or resume the original connection. It does not grant Windows logon rights. Agent connection interception and logon-rights mutation remain under development.
...
3. Assign a credential to each agentless node or group and verify WinRM access. Open a node to inspect collected identity, OS, network, DNS, firewall, and training details. New nodes enter automatic training; confirm WFP Connection success auditing on the host so traffic is observed.
...
- Before an agentless host finishes automatic training, WinFire reads the Windows Filtering Platform Connection audit policy and requires success auditing. The node detail shows this state and offers a read-only recheck. A disabled audit policy leaves training active with an actionable error. An administrator can explicitly enable success and failure auditing from the node detail; the WinRM command verifies readback and attempts to restore the prior setting if it fails. Group Policy can override the local setting. Success auditing can generate many Security events, especially on busy servers. [Microsoft documents this audit subcategory](https://learn.microsoft.com/en-us/windows/win32/fwp/auditing-and-logging).
...
The app was built on Linux. Live WMI/DCOM firewall operations and netsh-over-remote-shell fallback are **not implemented** in this build. The available Windows Server 2022 test host at `192.168.250.6` now exposes WinRM on TCP 5985. A temporary encrypted credential was used to collect system facts, page Security events, add scoped inbound and outbound rules for documentation address `192.0.2.1` and unused ports, verify their local and remote ports, confirm drift checks, replace the outbound port, and remove both through three audited policy apply runs. The remote group was empty after cleanup. The temporary database was deleted. The read-only `secedit` logon-rights export returned no rights after about 110 seconds, so baseline collection is still unverified and an empty export is rejected. WMI/DCOM also needs validation. No password for that host was stored in the repository.

The expanded host facts and full firewall-rule inventory have passed PowerShell mock tests, but a read-only retry against this Windows host returned HTTP 500 and then timed out. The host's initial rule listing exceeded a WSMan memory quota; the current paged implementation needs another live check. LDAPS and StartTLS were unavailable on the current test endpoint, so directory sync has been verified with a mock directory only. A trusted LDAPS domain controller and read-only bind account are needed for live AD validation.
